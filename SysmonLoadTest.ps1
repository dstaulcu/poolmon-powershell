...
function PoolMonSnap {
    ...
    if (Test-Path -Path $poshPoolmon) {
        $pool = & $poshPoolmon
        Add-Content -Path "$($ProgramDataPath)\poolsnap.log" -Value $pool
    }
}
...
function reset-sysmon ($sysmonPath, $configpath)
{
    ...
    Start-Process -FilePath $sysmonPath -ArgumentList @("-c --") -NoNewWindow | Out-Null
    Start-Sleep -Seconds 1
    Start-Process -FilePath $sysmonPath -ArgumentList @("-c",$configpath) -NoNewWindow | Out-Null
    Start-Sleep -Seconds 1
    PoolMonSnap
}
